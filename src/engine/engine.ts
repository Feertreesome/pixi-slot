...
import { CreationAudioPlugin } from "./audio/AudioPlugin.ts";
import { CreationNavigationPlugin } from "./navigation/NavigationPlugin.ts";
import { CreationResizePlugin } from "./resize/ResizePlugin.ts";
import { getResolution } from "./utils/getResolution.ts";
 ...
 *
 * This is a lightweight wrapper around the rendering application class.
 ...
 *
 * It also initializes the application and loads any assets in the `preload` bundle.
 */
export class CreationEngine extends Application {
  ...
  public async init(opts: Partial<ApplicationOptions>): Promise<void> {
    ...
    // Init assets with this asset manifest
